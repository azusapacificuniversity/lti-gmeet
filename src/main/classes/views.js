...
class Views {
  ...
  /**
     ...
     */
  getView(name, data) {
    if (!Object.keys(VIEWS).includes(name)) throw new Error(`Requested view ${name} does NOT exist.`);
  ...
  _getTemplate(tmplName) {
    if (Object.keys(this.compiled).includes(tmplName)) return this.compiled[tmplName];

    const _path = VIEWS[tmplName];
  ...
  _compileTemplate(_path) {
    const source = fs.readFileSync(path.resolve(_path)).toString();
